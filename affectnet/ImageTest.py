...
from affectnet.ImageHelpers import bbox2point
...
class TestData(Dataset):
    ...
    Args:
        testpath (str or list): Path to test data folder or list of image paths.
        iscrop (bool): Whether to crop the image based on detected face bounding box.
        crop_size (int): Size to which the image will be cropped.
        scale (float): Scaling factor for resizing the image.
        face_detector (str): Face detection method, currently supports 'fan'.
        scaling_factor (float): Scaling factor for image resizing.
        max_detection (int): Maximum number of detections to consider.
    """
...
def video2sequence(video_path):
    """
    Extracts frames from a video and stores them as individual images.
