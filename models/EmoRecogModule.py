...
import numpy as np
from utils.other import class_from_str  # Importing class_from_str function from utils.other module
...
import sys
import adabound
from affectnet.EmotionsData import ActionUnitTypes

# Function to create loss from configuration
...
# Base class for emotion recognition module using PyTorch Lightning
class EmotionRecognitionBaseModule(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        # Activation functions for valence, arousal, expression, and action units
        self.v_activation = class_from_str(self.config.model.v_activation, sys.modules[__name__]) if 'v_activation' in config.model.keys() else None
        self.a_activation = class_from_str(self.config.model.a_activation, sys.modules[__name__]) if 'a_activation' in config.model.keys() else None
        self.exp_activation = class_from_str(self.config.model.exp_activation, sys.modules[__name__]) if 'exp_activation' in config.model.keys() else F.log_softmax
        self.AU_activation = class_from_str(self.config.model.AU_activation, sys.modules[__name__]) if 'AU_activation' in config.model.keys() else None

        # Losses for valence, arousal, expression, and action units
    ...
    # Methods to check if valence, arousal, expression, or action units are predicted or trained
    def predicts_valence(self):
        return self.config.model.predict_valence

    def trains_valence(self):
        return self.config.model.v_loss not in [None, 'None', 'none', 0, False] and bool(self.config.model.v_loss)

    def predicts_arousal(self):
        return self.config.model.predict_arousal

    def trains_arousal(self):
        return self.config.model.a_loss not in [None, 'None', 'none', 0, False] and bool(self.config.model.a_loss)

    def predicts_expression(self):
        return self.config.model.predict_expression

    def trains_expression(self):
        return self.config.model.exp_loss not in [None, 'None', 'none', 0, False] and bool(self.config.model.exp_loss)
    ...
    def trains_AUs(self):
        return self.config.model.AU_loss not in [None, 'None', 'none', 0, False] and bool(self.config.model.AU_loss)
    ...
    # Method to configure optimizers
    ...
    # Method to calculate weights for valence-arousal loss terms
    ...
    # Method to compute loss and metrics
    ...
    # Method to compute loss during training
    ...
    # Method for logging losses and metrics
...
def v_or_a_loss(loss, pred, gt, term_weights, metrics, losses, measure, pred_prefix="", permit_dropping_corr=False, sample_weights=None):
    """
    Compute loss and metrics for valence or arousal.

    Args:
        loss: The loss function.
        pred: Predicted values.
        gt: Ground truth values.
        term_weights: Dictionary of loss term weights.
        metrics: Dictionary to store metrics.
        losses: Dictionary to store losses.
        measure: The measure to compute loss and metrics for, either "valence" or "arousal".
        pred_prefix: Prefix for predicted values in the dictionary.
        permit_dropping_corr: Whether to permit dropping correlation computation for a single sample.
        sample_weights: Weights for each sample.

    Returns:
        losses: Updated losses dictionary.
...
def va_loss(loss, pred, gt, weights, metrics, losses, pred_prefix="", permit_dropping_corr=False, sample_weights=None):
    """
    Compute loss and metrics for combined valence-arousal.
...
def exp_loss(loss, pred, gt, class_weight, metrics, losses, expression_balancing, num_classes, pred_prefix=""):
    """
    Compute loss and metrics for expression classification.

    Args:
        loss: The loss function.
        pred: Predicted values.
        gt: Ground truth values.
        class_weight: Weight for each class.
        metrics: Dictionary to store metrics.
        losses: Dictionary to store losses.
        expression_balancing: Whether to balance expression classes.
...
def AU_loss(loss, pred, gt, metrics, losses, AU_type, class_weights=None, pred_prefix=""):
    """
    Compute loss and metrics for action unit (AU) detection.
...
def _get_step_loss_weights(v_loss, a_loss, va_loss, scheme, training):
    """
    Compute loss weights for valence, arousal, and combined valence-arousal losses.

    Args:
        v_loss: Dictionary of valence losses.
        a_loss: Dictionary of arousal losses.
        va_loss: Dictionary of combined valence-arousal losses.
        scheme: The scheme to compute loss weights.
        training: Whether the model is in training mode.

    Returns:
        va_loss_weights: Dictionary of loss weights for valence, arousal, and combined valence-arousal losses.
    """
    va_loss_weights = {}
    for key in v_loss:
