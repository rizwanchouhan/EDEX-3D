...
from utils.LandmarkLists import left_eye_landmark_indices, right_eye_landmark_indices, mouth_landmark_indices
...
def get_mediapipe_indices():
    """
    Returns facial landmark indices used in the MediaPipe model.
    """
...
def batch_kp_2d_l1_loss(real_2d_kp, predicted_2d_kp, weights=None):
    """
    Compute L1 loss for 2D keypoints in batches.
    """
...
def mouth_corner_loss(predicted_landmarks, landmarks_gt, weights=None):
    """
    Compute mouth corner distance loss.
    """
...
def eye_dis(eye_upper, eye_lower):
    ...
    """
    dis = torch.sqrt(((eye_upper - eye_lower) ** 2).sum(2)) 
    return dis


def eyed_loss(predicted_landmarks, landmarks_gt, weights=None):
    """
    Compute eye distance loss.
    """
