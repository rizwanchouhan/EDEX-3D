...
from affectnet.AffectNetDataset import AffWild2DMBase
from affectnet.EmotionsModule import AffWild2DataModule
from pytorch_lightning import Trainer
from pytorch_lightning.callbacks import ModelCheckpoint, EarlyStopping
from models.DREAM import DecaModule, instantiate_deca
...
def get_checkpoint_with_kwargs(cfg, prefix, checkpoint_mode=None):
    """
    Get checkpoint and its associated keyword arguments.

    Args:
        cfg (DictConfig): Configuration dictionary.
        prefix (str): Prefix for the stage name.
        checkpoint_mode (str): Mode for selecting the checkpoint (default is None).

    Returns:
        tuple: Checkpoint and its associated keyword arguments.
    """
...
def locate_checkpoint(cfg, mode='latest'):
    """
    Locate the checkpoint based on the mode.

    Args:
        cfg (DictConfig): Configuration dictionary.
        mode (str): Mode for selecting the checkpoint (default is 'latest').
...
def prepare_data(cfg):
    """
    Prepare the data based on the configuration.

    Args:
        cfg (DictConfig): Configuration dictionary.

    Returns:
        tuple: Data module and sequence name.
    """
    ...
    annotation_list = OmegaConf.to_container(copy.deepcopy(cfg.data.annotation_list))  
...
def create_logger(logger_type, name, project_name, version, save_dir, config=None):
    """
    Create logger based on the provided parameters.
    ...
    Args:
        ...
        save_dir (str): Directory to save the logs.
        config (dict): Configuration parameters (default is None).

    Returns:
        WandbLogger: Created logger.
    """
...
def single_stage_deca_pass(deca, cfg, stage, prefix, dm=None, logger=None,
    ...
    """
    Single stage DECA pass.

    Args:
        deca (DecaModule): DECA model.
        cfg (DictConfig): Configuration dictionary.
        stage (str): Stage name.
        prefix (str): Prefix for the stage name.
        dm (AffWild2DataModule): Data module (default is None).
        logger (WandbLogger): Logger (default is None).
        data_preparation_function (function): Data preparation function (default is None).
        checkpoint (str): Checkpoint path (default is None).
        checkpoint_kwargs (dict): Checkpoint keyword arguments (default is None).
        project_name_ (str): Project name (default is None).
        instantiation_function (function): Instantiation function (default is None).

    Returns:
        DecaModule: DECA model.
    """
...
def create_experiment_name(cfg_coarse, cfg_detail, sequence_name, version=1):
    """
    Create an experiment name based on the configuration.

    Args:
        cfg_coarse (DictConfig): Coarse configuration.
        cfg_detail (DictConfig): Detail configuration.
        sequence_name (str): Sequence name.
        version (int): Naming version (default is 1).
    ...
    if version <= 1:
        ...
        # Add features if emonet loss is used
        ...
        # Add details about segmentation
        if cfg_detail.model.useSeg:
            experiment_name += f'_DeSeg{cfg_detail.model.useSeg}'
        else:
            experiment_name += f'_DeSeg{cfg_detail.model.useSeg}'
        # Add details about L1 and MRF
        ...
        # Add information about background
        ...
        # Add photometric, landmarks, and IDW
        if version == 0:
        ...
        # Add shape and detail constraint types
...
def finetune_deca(cfg_coarse, cfg_detail, test_first=True, start_i=-1, resume_from_previous=True,
                  force_new_location=False, project_name_=None):
    """
    Fine-tune DECA model.

    Args:
        cfg_coarse (DictConfig): Coarse configuration.
        cfg_detail (DictConfig): Detail configuration.
        test_first (bool): Whether to test first (default is True).
        start_i (int): Starting index (default is -1).
        resume_from_previous (bool): Whether to resume from previous (default is True).
        force_new_location (bool): Whether to force a new location (default is False).
        project_name_ (str): Project name (default is None).
    """
    project_name_ = project_name_ or project_name
...
def configure_and_finetune(coarse_cfg_default, coarse_overrides, detail_cfg_default, detail_overrides):
    """
    Configure and fine-tune DECA model.

    Args:
        coarse_cfg_default (str): Coarse configuration default.
        coarse_overrides (List[str]): Coarse overrides.
        detail_cfg_default (str): Detail configuration default.
        detail_overrides (List[str]): Detail overrides.
    """
...
def load_configs(run_path):
    """
    Load configurations from a run path.

    Args:
        run_path (str): Path to the run.

    Returns:
        Tuple[DictConfig, DictConfig]: Coarse and detail configurations.
    """
...
def configure_and_resume(run_path, coarse_cfg_default, coarse_overrides, detail_cfg_default, detail_overrides, start_at_stage):
    """
    Configure and resume DECA model.
    ...
    Args:
        ...
        detail_cfg_default (str): Detail configuration default.
        detail_overrides (List[str]): Detail overrides.
        start_at_stage (int): Starting stage.
    """
    cfg_coarse, cfg_detail = configure(coarse_cfg_default, coarse_overrides, detail_cfg_default, detail_overrides)
...
def resume_training(run_path, start_at_stage, resume_from_previous, force_new_location):
    """
    Resume training for a run.

    Args:
        run_path (str): Path to the run.
        start_at_stage (int): Starting stage.
        resume_from_previous (bool): Whether to resume from previous.
        force_new_location (bool): Whether to force a new location.
    """
    with open(Path(run_path) / "cfg.yaml", "r") as f:
        conf = OmegaConf.load(f)
    cfg_coarse = conf.coarse
