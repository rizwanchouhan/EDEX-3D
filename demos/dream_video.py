...
from affectnet.FaceVideoModule import TestFaceVideoDM
...
def reconstruct_video(args):
    ...
    # Setting up data module for testing
    ...
    for j, batch in enumerate (auto.tqdm( dl)):
        ...
        for i in range(current_bs):
            ...
            if args.save_mesh:
                save_obj(dream, str(sample_output_folder / "mesh_coarse.obj"), vals, i)
            if args.save_images:
                save_images(outfolder, name, visdict, i)
            if args.save_codes:
                save_codes(Path(outfolder), name, vals, i)

    # Creating reconstruction video
    ...
    # Logging the video if specified
    if args.logger == "wandb":
        from utils.DreamTest import  create_logger, project_name
        import wandb
...
def parse_args():
    ...
    parser.add_argument('--input_video', type=str, default= "/home/abbas/dream/input_video/Sad.mp4", 
        help="Filename of the video for reconstruction.")
    parser.add_argument('--output_folder', type=str, default="/home/abbas/dream/output_video", help="Output folder to save the results to.")
    parser.add_argument('--model_name', type=str, default='DREAM', help='Name of the model to use. Currently DREAM or DECA are available.')
    ...
    parser.add_argument('--image_type', type=str, default='geometry_detail', 
        choices=["geometry_detail", "geometry_detail", "out_im_detail", "out_im_coarse"], 
