...
class AffectNetExpressions(Enum):
    """
    Enumeration of expressions in the AffectNet dataset.
    """
    Neutral = 0
    Happy = 1
    Sad = 2
    ...
    def from_str(string : str):
        """
        Convert a string representation of an expression to the corresponding enum value.

        Args:
            string (str): String representation of the expression.

        Returns:
            AffectNetExpressions: Enum value corresponding to the input string.
        """
...
def make_class_balanced_sampler(labels):
    """
    Create a class-balanced sampler based on the input labels.

    Args:
        labels (array-like): Labels of the dataset samples.

    Returns:
        WeightedRandomSampler: Weighted random sampler for balancing classes.
    """
...
def make_va_balanced_sampler(labels):
    """
    Create a balanced sampler for valence-arousal labels.

    Args:
        labels (array-like): Valence-arousal labels of the dataset samples.

    Returns:
        WeightedRandomSampler: Weighted random sampler for balancing valence-arousal labels.
    """
    class_counts = np.bincount(labels)
    class_weights = 1. / class_counts
    weights = class_weights[labels]
...
def make_balanced_sample_by_weights(weights):
    """
    Create a balanced sampler based on provided weights.

    Args:
        weights (array-like): Weights for sampling.

    Returns:
        WeightedRandomSampler: Weighted random sampler based on the input weights.
    """
    return WeightedRandomSampler(weights, len(weights))

def new_affectnet(class_name):
    """
    Create a new instance of the specified AffectNet dataset class.

    Args:
        class_name (str): Name of the dataset class.

    Returns:
        dataset_class: Instance of the specified dataset class.
    """
    dataset_class = class_from_str(class_name, sys.modules[__name__])
    return dataset_class


class AffectNetModule(FaceBaseModule):
    """
    AffectNetModule implements a wrapper for the AffectNet dataset
    """
    def __init__(self,
                 input_dir,
                 output_dir,
        ...
        if mode == "manual":
            ...
            print("hhedfsdfsfdfre", validation_fname)
            self.test_fname = test_fname or "validation_representative_selection.csv"

            training_fname = '/home/abbas/dream/assets/data/affectnet/Manually_Annotated/training.csv'
            validation_fname = "/home/abbas/dream/assets/data/affectnet/Manually_Annotated/validation.csv"

            train = pd.read_csv(training_fname)
            val = pd.read_csv(validation_fname)
            self.df = pd.concat([train, val], ignore_index=True, sort=False)
            self.train_dataframe_path = Path(self.root_dir) / (self.mode_str + "_Annotated") / training_fname
            self.val_dataframe_path = Path(self.root_dir) / (self.mode_str + "_Annotated") / validation_fname
        elif mode == "automatic":
    ...
    def _detect_landmarks_and_segment_subset(self, start_i, end_i):
        ...
        if not completed:
            ...
            for i in auto.tqdm(range(start_i, end_i)):
                ...
                print("here1", im_file)
                print("here2", Path(self.input_dir))
    ...
    def _detect_landmarks_mediapipe(self, start_i, end_i):
        ...
        completed = status_array[start_i // self.subset_size]
        del status_array
        from utils.FaceDetector import save_landmark_v2, save_landmark
...
class AffectNetEmoNetSplitModule(AffectNetModule):
    """
    AffectNetEmoNetSplitModule implements a wrapper for the AffectNet dataset with EmoNet split.
    """

    def __init__(self, *args, ignore_invalid=True, **kwargs):
        """
        Initialize the AffectNetEmoNetSplitModule.

        Args:
            *args: Variable length argument list.
            ignore_invalid (bool or str): Flag indicating whether to ignore invalid samples or not.
            **kwargs: Arbitrary keyword arguments.
        """
        training_fname = "training_emonet_split.csv"
    ...
    def setup(self, stage=None):
        """
        Set up the AffectNetEmoNetSplitModule.

        Args:
            stage: Stage of the setup process.
        """
        self.training_set = self._new_training_set()
        self.validation_set = new_affectnet(self.dataset_type)(self.image_path, self.val_dataframe_path, self.image_size, self.scale,
                                        None, ignore_invalid=self.ignore_invalid,
    ...
    def val_dataloader(self):
        ...
        Returns:
            list: List of validation dataloaders.
        """
...
class AffectNetEmoNetSplitModuleTest(AffectNetModule):
    """
    AffectNetEmoNetSplitModuleTest implements a wrapper for the AffectNet dataset with EmoNet split for testing.
    """

    def __init__(self, *args, **kwargs):
        """
        Initialize the AffectNetEmoNetSplitModuleTest.
    ...
    def setup(self, stage=None):
        """
        Set up the AffectNetEmoNetSplitModuleTest.
    ...
    def train_dataloader(self):
        ...
        Returns:
            NotImplementedError: Training dataloader is not implemented.
        """
        raise NotImplementedError("Not implemented")

class AffectNetEmoNetSplitModuleValTest(AffectNetEmoNetSplitModule):
    """
    AffectNetEmoNetSplitModuleValTest implements a wrapper for the AffectNet dataset with EmoNet split for validation and testing.
    """

    def __init__(self, *args, **kwargs):
        """
        Initialize the AffectNetEmoNetSplitModuleValTest.
    ...
    def setup(self, stage=None):
        """
        Set up the AffectNetEmoNetSplitModuleValTest.
...
class AffectNetTestModule(AffectNetModule):
    """
    AffectNetTestModule implements a wrapper for the AffectNet dataset for testing.
    """

    def prepare_data(self):
        """
        Prepare data for the AffectNetTestModule.
        """
    ...
    def setup(self, stage=None):
        """
        Set up the AffectNetTestModule.
        ...
        if self.use_processed:
            ...
            self.image_path = Path(self.output_dir) / (self.mode_str + "_Annotated") / (
                        self.mode_str + "_Annotated_Images")
        self.test_set = new_affectnet(self.dataset_type)(self.image_path, self.test_dataframe_path, self.image_size,
                                                          self.scale,
                                                          None, ignore_invalid=self.ignore_invalid,
                                                          use_gt=self.use_gt,)
    ...
    def val_dataloader(self):
        ...
        Returns:
            None: No validation dataloader for testing.
        """
        return None
...
class AffectNetEmoNetSplitTestModule(AffectNetTestModule):
    """
    AffectNetEmoNetSplitTestModule implements a wrapper for the AffectNet dataset with EmoNet split for testing.
    """

    def setup(self, stage=None):
        """
        Set up the AffectNetEmoNetSplitTestModule.

        Args:
            stage: Stage of the setup process.
        """
        self.test_dataframe_path = Path(self.root_dir) / (self.mode_str + "_Annotated") / \
                                   "validation_emonet_split_clean_representative.csv"
        if self.use_processed:
            self.image_path = Path(self.output_dir) / "detections"
        else:
            self.image_path = Path(self.root_dir) / (self.mode_str + "_Annotated") / (
                        self.mode_str + "_Annotated_Images")
        self.test_set = new_affectnet(self.dataset_type)(self.image_path, self.test_dataframe_path, self.image_size,
                                                          self.scale,
                                                          None, ignore_invalid=self.ignore_invalid,
...
class AffectNetEmoNetSplitTestModule2(AffectNetTestModule):
    """
    AffectNetEmoNetSplitTestModule2 implements a wrapper for the AffectNet dataset with EmoNet split for testing.
    """

    def setup(self, stage=None):
        """
        Set up the AffectNetEmoNetSplitTestModule2.

        Args:
            stage: Stage of the setup process.
        """
        self.test_dataframe_path = Path(self.root_dir) / (self.mode_str + "_Annotated") / \
                                   "validation_representative_selection.csv"
        if self.use_processed:
            self.image_path = Path(self.output_dir) / "detections"
        else:
            self.image_path = Path(self.root_dir) / (self.mode_str + "_Annotated") / (
...
class AffectNetDataModuleValTest(AffectNetModule):
    """
    AffectNetDataModuleValTest implements a wrapper for the AffectNet dataset for validation and testing.
    """

    def __init__(self, *args, **kwargs):
        """
        Initialize the AffectNetDataModuleValTest.
    ...
    def setup(self, stage=None):
        """
        Set up the AffectNetDataModuleValTest.

        Args:
            stage: Stage of the setup process.
        """
        self.training_set = self._new_training_set()
        self.validation_set = new_affectnet(self.dataset_type)(self.image_path, self.val_dataframe_path, self.image_size,
                                                                self.scale,
                                                                None, ignore_invalid=self.ignore_invalid,
                                                                ring_type=self.ring_type,
                                                                ring_size=1,
        ...
        self.test_set = new_affectnet(self.dataset_type)(self.image_path, self.val_dataframe_path, self.image_size,
                                                          self.scale,
                                                          None, ignore_invalid=self.ignore_invalid,
                                                          ring_type=self.ring_type,
                                                          ring_size=1,
...
class AffectNet(EmotionalImageDatasetBase):
    """
    AffectNet implements a dataset class for the AffectNet dataset.
    """
    ...
    def __init__(self,
        ...
        """
        Initialize the AffectNet dataset.

        Args:
            image_path (str): Path to the images.
            dataframe_path (str): Path to the dataframe containing dataset information.
            image_size (int): Size of the images.
            scale (float): Scale factor.
            transforms (imgaug.augmenters.Augmenter): Image augmentation transforms.
            use_gt_bb (bool): Whether to use ground truth bounding boxes.
            ignore_invalid (bool): Whether to ignore invalid data.
            ring_type (str): Type of ring.
            ring_size (int): Size of the ring.
            load_emotion_feature (bool): Whether to load emotion features.
            nn_indices_array: Array of nearest neighbor indices.
            nn_distances_array: Array of nearest neighbor distances.
            ext (str): File extension of the images.
            use_gt (bool): Whether to use ground truth data.
        """
        ...
        self.load_mediapipe_landmarks = True
    ...
    def _init_sample_weights(self):
        ...
        if self.ring_type == "emonet_feature":
            if len(self) != self.nn_distances_array.shape[0] or len(self) != self.nn_indices_array.shape[0]:
                raise RuntimeError(
                    "The lengths of the dataset does not correspond to size of the nn_array. "
...
class AffectNetWithPredictions(AffectNet):
    """
    AffectNetWithPredictions extends AffectNet dataset class with additional predictions.
    """

    def __init__(self, predictor, shape_name, exp_name, *args, **kwargs):
        """
        Initialize AffectNetWithPredictions.

        Args:
            predictor (str): Name of the predictor.
            shape_name (str): Name of the shape prediction.
            exp_name (str): Name of the expression prediction.
    ...
    def _load_additional_data(self, im_rel_path):
        ...
        Args:
            im_rel_path (str): Relative path of the image.
...
class AffectNetWithMGCNetPredictions(AffectNetWithPredictions):
    """
    AffectNetWithMGCNetPredictions extends AffectNetWithPredictions with MGCNet predictions.
    """

    def __init__(self, *args, **kwargs):
        """
        Initialize AffectNetWithMGCNetPredictions.
...
class AffectNetWithExpNetPredictions(AffectNetWithPredictions):
    """
    AffectNetWithExpNetPredictions extends AffectNetWithPredictions with ExpNet predictions.
    """

    def __init__(self, *args, **kwargs):
        """
        Initialize AffectNetWithExpNetPredictions.
...
class AffectNetWithExpNetPredictionsMyCrop(AffectNetWithPredictions):
    """
    AffectNetWithExpNetPredictionsMyCrop extends AffectNetWithPredictions with ExpNet predictions for crops.
    """

    def __init__(self, *args, **kwargs):
        """
        Initialize AffectNetWithExpNetPredictionsMyCrop.
...
def sample_representative_set(df, output_file, sample_step=0.1, num_per_bin=2):
    """
    Sample representative set from the dataframe and save to file.

    Args:
        df (DataFrame): Input dataframe.
        output_file (str): Output file path.
        sample_step (float): Sampling step.
        num_per_bin (int): Number of samples per bin.
    """
    va_array = []
...
def emonet_split_pkl_to_csv():
    """
    Convert EmoNet split pickle files to CSV format.
    """
    with open(Path("/home/abbas/dream/Workspace/Repos/emonet/pickles/val_fullpath.pkl"), "rb") as f:
        emonet_val_set = pkl.load(f)
    with open(Path("/home/abbas/dream/Workspace/Repos/emonet/pickles/test_fullpath.pkl"), "rb") as f:
        emonet_test_set = pkl.load(f)

    train_df_path = "/home/abbas/dream/assets/data/affectnet/Manually_Annotated/training.csv"
    new_train_df_path = "/home/abbas/dream/EmotionalFacialAnimation/data/affectnet/Manually_Annotated/training_emonet_split.csv"
    new_val_df_path = "/home/abbas/dream/EmotionalFacialAnimation/data/affectnet/Manually_Annotated/validation_emonet_split.csv"
    new_subset_val_df_path = "/home/abbas/dream/EmotionalFacialAnimation/data/affectnet/Manually_Annotated/validation_representative_emonet_split.csv"
...
def replace_labels(df, dic):
    ...
    Args:
        df (DataFrame): Input dataframe.
        dic (dict): Dictionary containing replacement values.
...
def emonet_split_cleaned_label_transfer():
    """
    Transfer cleaned labels from EmoNet split to AffectNet dataset.
    """
    with open(Path("/home/abbas/dream/Workspace/Repos/emonet/pickles/val_fullpath.pkl"), "rb") as f:
        emonet_val_set = pkl.load(f)
    with open(Path("/home/abbas/dream/Workspace/Repos/emonet/pickles/test_fullpath.pkl"), "rb") as f:
    ...
    new_val_df_path = "/home/abbas/dream/EmotionalFacialAnimation/data/affectnet/Manually_Annotated/validation_emonet_split.csv"
    new_val_df_cleaned_path = "/home/abbas/dream/EmotionalFacialAnimation/data/affectnet/Manually_Annotated/validation_emonet_split_clean.csv"
    new_val_df_cleaned_repre_path = "/home/abbas/dream/EmotionalFacialAnimation/data/affectnet/Manually_Annotated/validation_emonet_split_clean_representative.csv"
    ...
    new_test_df_path = "/home/abbas/dream/EmotionalFacialAnimation/data/affectnet/Manually_Annotated/testing_emonet_split.csv"
    new_test_df_cleaned_path = "/home/abbas/dream/EmotionalFacialAnimation/data/affectnet/Manually_Annotated/testing_emonet_split_clean.csv"
...
if __name__ == "__main__":
    import yaml
    augmenter = yaml.load(
        open(Path(__file__).parents[2] / "gdl_apps" / "EmotionRecognition" / "emodeca_conf" / "data" / "augmentations" /
             "default_with_resize.yaml"),
        Loader=yaml.FullLoader)["augmentation"]
    dm = AffectNetEmoNetSplitModuleValTest(
        "/home/abbas/dream/EmotionalFacialAnimation/data/affectnet/",
        "/home/abbas/data/affectnet/",
